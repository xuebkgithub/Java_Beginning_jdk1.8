...
//  }
  ...
  public void withdraw(double atm) throws OverdraftException {
    try {
      ...
      this.Checkingwithdraw(atm);
    }


  }

  private void Checkingwithdraw(double atm) throws OverdraftException {
    if (this.overdraftProtection == null){
      throw new  OverdraftException("not overdraft Exception",atm - this.getBalance());
    }
    if ( this.overdraftProtection >= atm - this.getBalance()){
      this.overdraftProtection -= ( atm - this.getBalance());
      this.setBalance(0);
    }else {
      throw new OverdraftException("信用卡 余额小于您要取得钱", atm - balance - this.overdraftProtection);
    }
